...
class ProductList extends React.Component {
    ...
    render() {
        let itemsElements = this.props.items.map((item, index) => {
          return (
            <li key={index}>
                <button className={styles.searchlist} onClick={(event) => {this.showDetail(event, item)}}> {item.name} </button>
            </li>
            );
    ...
    return(
      <div className={styles.container}>
        <div className={styles.col}>
                <h3>Search:</h3>
                <input className={styles.search} onChange={(event)=>{this.changeHandler(event);}} value={this.state.searchText}/>
